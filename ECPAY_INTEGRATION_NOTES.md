...
- `ReturnURL` is handled by `/api/ecpay-notify` for server-side notifications and must respond with `1|OK` after checksum verification.
- `OrderResultURL` is handled by `/api/ecpay-return` and redirects the browser back to `/test/ecpay` with result parameters.
